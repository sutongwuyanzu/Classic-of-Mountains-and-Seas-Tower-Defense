...
assert.match(indexHtml, /id="auto-skill"/);
assert.match(indexHtml, /id="selected-skill-mode"/);
...
assert.match(desktopGame, /function difficultyUnlocked\(stage, difficulty\)/);
assert.match(desktopGame, /function toggleGlobalSkillMode\(\)/);
assert.match(desktopGame, /function cycleSelectedSkillMode\(\)/);
assert.match(desktopGame, /function summonHighlightFor\(beastId\)/);
assert.match(desktopGame, /if \(state\.phase === 'combat' && !state\.pendingTargetSkillUid && skillAutoModeFor\(tower\) && castSpiritSkill\(tower\)\) return;/);
assert.match(desktopGame, /\['spirit', 3\]/);
...
assert(Core.medalAwards({ won:true, hp:9, maxHp:10, difficulty:'easy', urCount:1, maxRarity:4, activeBonds:0, requiredFielded:false, maxGrowthKills:0, clearedAll:false, clearedHardAll:false, unlockedNormal:true, unlockedHard:true, mode:'standard', wave:1 }).includes('normal_unlocked'));
assert(Core.medalAwards({ won:true, hp:9, maxHp:10, difficulty:'easy', urCount:1, maxRarity:4, activeBonds:0, requiredFielded:false, maxGrowthKills:0, clearedAll:false, clearedHardAll:false, unlockedNormal:true, unlockedHard:true, mode:'standard', wave:1 }).includes('hard_unlocked'));
assert(!Core.medalAwards({ won:false, hp:0, maxHp:10, difficulty:'easy', urCount:1, maxRarity:4, activeBonds:0, requiredFielded:false, maxGrowthKills:0, clearedAll:false, clearedHardAll:false, unlockedNormal:true, unlockedHard:true, mode:'standard', wave:1 }).includes('normal_unlocked'));
